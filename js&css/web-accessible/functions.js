...
ImprovedTube.extractSubscriberCount = function (subscriberCountNode) {
	...
	if (subscriberCountNode) {
		var abbreviatedText = subscriberCountNode.textContent.trim();
		var fullCount = null;

		// Try to get full count from data attributes or API
		if (subscriberCountNode.dataset && subscriberCountNode.dataset.subscriberCount) {
			fullCount = subscriberCountNode.dataset.subscriberCount;
		} else {
			// Fallback: Try to parse from abbreviated text
			fullCount = parseFloat(abbreviatedText.replace(/[^0-9.]/g, ''));
			if (abbreviatedText.includes('K')) {
				fullCount *= 1000;
			} else if (abbreviatedText.includes('M')) {
				fullCount *= 1000000;
			}
		}
		...
		if (ImprovedTube.storage.show_full_subscriber_count === true && fullCount) {
			subscriberCountNode.textContent = fullCount.toLocaleString();
		}
	}
};
